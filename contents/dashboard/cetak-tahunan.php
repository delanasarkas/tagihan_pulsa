...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(80);
		//judul
        $this->Cell(75.5,15,'REPORT PENGHASILAN TAHUNAN MASIKA RELOAD PADA TAHUN '.strtoupper(date('Y')),0,1,'C');
	...
	function Content()
	{
		...
		if($rolle == 'admin'){
            ...
            AND YEAR(a.tgl_setoran) = YEAR(NOW());");
		}
...
$pdf = new PDF('L','mm','A4');
$pdf->SetTitle('Report Tahunan');
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->Content();
$pdf->Output("reporttahunan_".strtoupper(date('Y')).".pdf","I");
?>